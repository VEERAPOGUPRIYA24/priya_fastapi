...
app = FastAPI(title="Mobile Phone Management System")
def get_db():
    db = SessionLocal()
    try:
        yield db
...
@app.get("/")
def welcome():
    return {"message": "Welcome to Mobile Phone Management System"}
...
@app.get("/brand/{brand_name}", response_model=list[schemas.MobileResponse])
def mobiles_by_brand(brand_name: str, db: Session = Depends(get_db)):
    mobiles = crud.get_by_brand(db, brand_name)
    if not mobiles:
        raise HTTPException(status_code=404, detail="No mobiles found for this brand")
    return mobiles